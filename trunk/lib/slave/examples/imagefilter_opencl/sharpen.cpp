...
#define SHARPEN_PATH "sharpen.cl"
...
int do_calc_OpenCL()
{
  ...
  err = clEnqueueWriteImage(queue, input, CL_TRUE, origin, region, image_width*16,0,image2,0,NULL,NULL); 
  ...
  for(x=1; x<=image_width-1; x++)
  {
	  ...
	  */
	  ...
	  clSetKernelArg(kernel, 1, sizeof(cl_mem), &outputr);
	  clSetKernelArg(kernel, 2, sizeof(cl_mem), &outputg);
	  clSetKernelArg(kernel, 3, sizeof(cl_mem), &outputb);
	  clSetKernelArg(kernel, 4, sizeof(cl_int), &x);
